...
| `rockchip,high-temp = <95000>` and `rockchip,high-temp-max-volt = <1100000>` on `cpu0_opp_table` | Without them the system monitor leaves its threshold at `INT_MAX` and never engages, so the thermal governor regulating to the 85 &deg;C trip is the only limit. Above 95 &deg;C these cap the CPU to the operating points that need no more than 1.1 V. This board has no heatsink, so the die has nowhere to dump heat when the ambient itself is hot. Taken from `rk3562.dtsi` in the Rockchip `develop-6.1` branch; the 5.10 system monitor already parses both properties. |
